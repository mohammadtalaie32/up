...
<a href="{{ route('client.create') }}">Add client</a> | Admin<br><br>
<script>
...
function managesubmit(username){
...
<b>Existing users</b><br><br><form name="userform" action="adminuser.php" method="post"><input type="hidden" name="user"><input type="hidden" name="operation">
<table cellspacing="0" cellpadding="5" border="1"><tbody><tr><td>Manage</td><td>Name</td><td>User Type</td><td>Email</td><td>Patient ID</td><td>Forms</td></tr><tr><td align="center"><img src="{{ asset('nlimages/manageuser.png') }}" onclick="managesubmit('demo');"></td><td>Demo User</td><td>admin</td><td></td><td></td><td><a href="{{ URL::to('forms') }}">Forms</a></td></tr></tbody></table></form>
<br><b>Pending users</b><br><br><table cellspacing="0" cellpadding="5" border="1"><tbody><tr>
	<td>Name</td><td>Patient ID</td><td>Temp Password</td><td>Invite Email</td><td>Registration</td><td>
	@foreach($records as $record)
	</td></tr><tr>

		<td>{{$record->firstname }} {{$record->lastname}} </td><td>{{$record->id}}</td><td>{{$record->password}}</td><td><a href="">Resend</a></td><td><a href="">Open Form</a></td><td><a href="{{URL::to('/delete/'.$record->id) }}" onclick="return confirm('Are you sure you want to delete this patient?');"">Remove</a></td>
	@endforeach
	</tr></tbody></table><br>
<b>Patients without user accounts:</b><br><br>
Opening a registration form or sending an invite link will create a temporary password / invite code for them to create an account linked to the existing patient.<br><br>

	<table cellspacing="0" cellpadding="5" border="1">
	<tbody><tr><td>Name</td><td>Patient ID</td><td>Email</td><td>Register Form</td><td>Send Invite</td></tr>
	...
	<tr>
		...
		<td>{{ $record->id }}</td>
		<td>{{ $record->email }}</td>
		<td><a href="{{ route('registernewuser') }}">Start Registration</a>
		</td>
		<td><a href="">Email</a>
		</td>
		@endforeach
...
<br><br><a href="">Send invite link to existing patient</a><br><br>
<a href="{{ route('registernewuser') }}">Open user registration page</a> (to be used in person)    </td></tr></tbody></table></td></tr></tbody></table>
</form></body></html>
